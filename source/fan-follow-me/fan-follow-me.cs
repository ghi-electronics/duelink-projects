// In this project:
// Use a distance sensor to scan for and detect objects.
// If an object is detected closer than 15 cm, rotate the fan toward the object and turn the fan on.
// While the object is present within 15 cm, keep the fan on.
// If the object disappears or moves farther than 15 cm, turn the fan off.
// Note: The fan and two servo motors draw much power (higher 500mA) and may require an external power source using a Power Inject module:
// https://www.duelink.com/docs/products/adpwrinj-a
// Device setup:
// servo motor : address 1
// distance: address 2
// servo motor: address 3
// fan: address 4

using System.Diagnostics;
using GHIElectronics.DUELink;

var availablePort = DUELinkController.GetConnectionPort();
var duelink = new DUELinkController(availablePort);

var distance = 0;
var angle = 0;
var offset = 5;
var dir = 1;
var current_dev = 0;

void SetAddress(int add) {
    if (current_dev != add) {
        current_dev = add;
        duelink.Engine.ExecuteCommand($"sel({current_dev})");
    }
}

void SetServo(int servo, int angle) {
    // Servo 1: control distance degree
    // Servo 3: control fan degree
    SetAddress(servo);
    duelink.Engine.ExecuteCommand($"servost(1,{angle})");
}

void SetFanSpeed(int speed, int angle) {
    if (angle != -1)
        SetServo(3, angle);

    SetAddress(4);
    duelink.Engine.ExecuteCommand($"fan({speed})");
}

int Distance() {
    SetAddress(2);

    var d = (int)duelink.Engine.ExecuteCommand("distance()");

    return d;
}

int Scan(int angle) {
    SetServo(1, angle);

    return Distance();
}

void Setup() {
    SetServo(1, 20);
    SetFanSpeed(0, 20);
}

void Loop() {
    while (true) {
        Thread.Sleep(100);
        distance = Distance();
        SetServo(1, angle);

        angle += (dir * offset);

        if (angle > 160) {
            dir = -1;
        }

        if (angle < 20) {
            dir = 1;
        }

        if (distance < 15 && distance > 0) {
            SetFanSpeed(60, angle);

            while (distance < 15) {
                distance = Distance();
                Thread.Sleep(80);
            }

            SetFanSpeed(0, -1);

        }
    }
}

Setup();
Loop();

